...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const createDefaultOrganization = async (user: User) => {
    ...
    const response = await fetch(`${API_URL}/api/v1/getOrganizations`, {
      method: 'POST',
      headers: {
  ...
  const loadOrganizations = async (user: User) => {
    try {
      const token = await user.getIdToken();
      const response = await fetch(`${API_URL}/api/v1/getOrganizations?page=1&pageSize=100`, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
  ...
  const registerWithEmail = async (email: string, password: string, organizationName: string) => {
    try {
      ...
      const orgResponse = await fetch(`${API_URL}/api/v1/getOrganizations`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
